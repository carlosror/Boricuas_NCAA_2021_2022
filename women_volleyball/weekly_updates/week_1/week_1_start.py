...
players_list = []
current_week = "1"
with open("players_of_the_week_" + current_week + ".csv") as f:
  ...
  for line in f:
    ...
    award_date = current_line_list[6]
...
if newDocument(PAPER_LETTER, margins, PORTRAIT, 1,  UNIT_POINTS, NOFACINGPAGES, FIRSTPAGERIGHT, 1):
  ...
  for current_player in players_list:
    ...
    loadImage(player_image, player); setScaleImageToFrame(1, 1, player)
    ...
    if (current_player[5].replace("\n","") in ["NCAA DI", "NCAA DII", "NCAA DIII", "NJCAA DI", "NJCAA DII"]):
      ...
      player_division = createImage(15, division_y, 65, 65)
    else:
      division_y = (award_rect_height + conf_rect_height) + 20
      player_division = createImage(15, division_y, 65, 32)
    ...
    conf_logo = createImage(15, (award_rect_height + conf_rect_height + 65 + 30), player_conf_logo_w, player_conf_logo_h)
    ...
    player_name = createText(100, player_name_ypos - 1, 514, 75)
